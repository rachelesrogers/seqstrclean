...
fnc_printable <- fnc %>% slice(-1) %>% select(-c(algorithm,page_notes,clean_prints)) %>% head(3)
...
extreme_printable <- head(extreme_dataset %>% filter(extreme_value==TRUE, clean_prints==39) %>% subset(select=c("notes", "corrected_notes", "page_notes", "extreme_value")), 1)
...
extreme_printable2 <- head(extreme_dataset %>% filter(apply_lcs==TRUE, clean_prints==39) %>% subset(select=c("notes", "page_notes", "apply_lcs")), 1)
...
hybrid_printable <- head(hybrid_dataset[hybrid_dataset$apply_lcs==TRUE,] %>% subset(select=c("notes", "page_notes", "lcs_notes")), 1)
